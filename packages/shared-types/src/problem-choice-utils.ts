...
export interface ChoiceSourceLike {
...
function findInlineChoiceMarkers(
  ...
  if (markerStyle === "latex-aware") {
    return Array.from(value.matchAll(LATEX_AWARE_INLINE_CHOICE_PATTERN)).map(
      (match) => ({
        ...
        marker: match[2] ?? "",
        position: resolveChoiceMarkerPosition(match[2] ?? "", markerStyle),
      }),
    );
  }
  ...
  return Array.from(value.matchAll(BASIC_INLINE_CHOICE_PATTERN)).map((match) => ({
    index: match.index ?? 0,
    marker: match[0],
    position: resolveChoiceMarkerPosition(match[0], markerStyle),
  }));
}

export function resolveChoiceMarkerPosition(
  marker: string,
  _markerStyle: ChoiceMarkerStyle = "basic",
): number {
