...
describe("SubmissionsService", () => {
  ...
  const mockClassMonitorGateway = {
    emitSubmission: jest.fn(),
  };

  const mockClassMonitorService = {
    getStatus: jest.fn().mockResolvedValue({ students: [], alerts: [] }),
  };
  ...
  it("rejects answers for problems that are not in the assignment", async () => {
    ...
    const service = new SubmissionsService(prisma as never, mockWrongAnswers as never, mockMastery as never, mockGamification as never, mockSmartScore as never, mockClassMonitorGateway as never, mockClassMonitorService as never);
